...
static inline void *CgoWebViewCreate(int width, int height, char *title, char *url, int resizable, int debug, int borderless) {
...
// (Linux/MacOS).
func Open(title, url string, w, h int, resizable bool, borderless bool) error {
	...
	r := C.webview(titleStr, urlStr, C.int(w), C.int(h), resize, border)
	if r != 0 {
...
func New(settings Settings) WebView {
	...
	w.w = C.CgoWebViewCreate(C.int(settings.Width), C.int(settings.Height),
		...
		C.int(boolToInt(settings.Resizable)), C.int(boolToInt(settings.Debug)), C.int(boolToInt(settings.Borderless)))
	m.Lock()
